  ...
  return (
    <header
      className={`fixed w-full z-50 transition-all duration-300 ${
        isScrolled
          ? 'bg-white shadow-md py-2 dark:bg-gray-950 dark:shadow-gray-900/50'
          : 'bg-white/95 backdrop-blur-sm py-4 dark:bg-gray-950/95'
      }`}
    >
      <div className="container mx-auto px-4 md:px-6">
        <nav className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              <span className="text-xl md:text-2xl font-bold font-serif-sc text-zk-red group-hover:text-zk-blue transition-colors dark:text-zk-red">
                {SCHOOL_INFO.name}
              </span>
              ...
              return (
                <Link
                  ...
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200
                    ${isActive
                      ? 'text-zk-red bg-red-50 dark:text-zk-red dark:bg-red-950/30'
                      : 'text-gray-700 hover:text-zk-red hover:bg-gray-50 dark:text-gray-300 dark:hover:text-zk-red dark:hover:bg-gray-800'
                    }`}
                >
                  {item.label}
                </Link>
          ...
          <div className="lg:hidden flex items-center gap-2">
            <ThemeSwitch />
            <button
              className="text-gray-700 dark:text-gray-300 hover:text-zk-red focus:outline-none"
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-300 ease-in-out ${
          ...
          <div className="flex flex-col space-y-2 bg-gray-50 dark:bg-gray-900 p-4 rounded-lg shadow-inner">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.path}
                href={item.path}
                className={`block px-4 py-3 rounded-md text-base font-medium transition-colors ${
                  pathname === item.path
                    ? 'text-zk-red bg-white dark:bg-gray-800'
                    : 'text-gray-700 hover:text-zk-red hover:bg-white dark:text-gray-300 dark:hover:bg-gray-800'
                }`}
              >
                {item.label}
              </Link>
